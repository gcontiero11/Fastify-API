...
class DiscountEngine {
  ...
  private applyCategoryDiscountBy(item: Item) {
    ...
    if (isAccessory && meetsQuantity) {
      console.log(`   Categoria: ${item.getCategory()}, Quantidade: ${item.getQuantity()}`);
  ...
  private applyQuantityDiscount(order: Order) {
    ...
    if (quantity >= 50) {
      console.log(`🔍 Aplicando desconto por quantidade: ${quantity} itens >= 50`);
      this.applyDiscountIntoOrder(order, "QTY_TIE_20PCT");
    } else if (quantity >= 20) {
      console.log(`🔍 Aplicando desconto por quantidade: ${quantity} itens >= 20`);
      this.applyDiscountIntoOrder(order, "QTY_TIE_15PCT");
    } else if (quantity >= 10) {
      console.log(`🔍 Aplicando desconto por quantidade: ${quantity} itens >= 10`);
  ...
  private applyDiscountIntoItem(item: Item, code: string) {
    ...
    console.log(`💰 Aplicando desconto ${code} de ${discount.getRate() * 100}% no item ${item.getProductId()}:`);
  ...
  private applyDiscountByCartValue(order: Order) {
    ...
    if (cartValue >= 2000) {
      console.log(`🔍 Verificando desconto por valor do carrinho: R$ ${cartValue} >= R$ 2000`);
      this.applyDiscountIntoOrder(order, "CART_VALUE_FIXED_150");
    } else if (cartValue >= 1000) {
      console.log(`🔍 Verificando desconto por valor do carrinho: R$ ${cartValue} >= R$ 1000`);
      this.applyDiscountIntoOrder(order, "CART_VALUE_FIXED_50");
    } else {
      console.log(`🔍 Valor do carrinho R$ ${cartValue} não atende aos critérios de desconto por valor do carrinho`);
    }
  }
